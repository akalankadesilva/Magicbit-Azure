...
void configServerInit(){
  configServer.on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
    ...
    Serial.print("NW count:");
    Serial.println(nwCount);
    ...
    sprintf(response, indexHtml, s6,s1,s2,s5,mConfig.deviceId,mConfig.hub, s3,s4,s7);
  ...
  configServer.on("/submit", HTTP_POST, [](AsyncWebServerRequest * request) {
    ...
    if (request->hasParam("p", true)) {
      ...
      if (p==String(mConfig.adminPassword)) {
        ...
        h = request->getParam("h", true)->value();
        ...
        if(strcmp(mConfig.hub,h.c_str())!=0){
          dirty=true;
          Serial.print("New Hub name:");
          strcpy(mConfig.hub,h.c_str());
          Serial.println(mConfig.hub);
        }
